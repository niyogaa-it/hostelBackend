...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (randomRoom.length == 0) {
      ...
      var total_vacant = randomRoom[0].total_occupancy - get_room_data.length;

    
      await knex("room")
      .where({ id: randomRoom[0].id })
      .update({
      ...
      if (randomRoom.length == 0) {
        ...
        .update({
          ...
          if (req.body.parking == "Yes") {
            ...
            if (isParkingExists.length == 1) {
              await knex("parking_slot").where({ id: isParkingExists[0].id }).update({
                is_alloted: 1,
                student_id: req.body.student_id,
              });
              ...
              var isSaveData = await knex("student_details")
                ...
                .update({
                  ...
                  parking_name: isParkingExists[0].parking_name,
...
router.put("/deactivate/:id", async (req, res) => {
  ...
  try {
    let isStudentidExists = await knex("student_details")
      .where({
        id: req.params.id,
        status: 1,
        is_approved: 1,
    ...
    if (isStudentidExists.length == 0) {
      ...
      if(isStudentidExists[0].room_id!=0){


         let total_vacant = 0;
            //leave_room
          let get_room_details = await knex("room")
          .where({
            id: isStudentidExists[0].room_id,
          });
      ...
      let get_room_data = await knex("room_occupancy").where({
        room_number: isStudentidExists[0].room_id,
        status: 1,
      });


      if(get_room_data.length>0){

        total_vacant = get_room_details[0].total_occupancy - get_room_data.length;
      }else{
        total_vacant = get_room_details[0].total_occupancy;

      }


      var update_room = await knex("room")
      .where({ id: isStudentidExists[0].room_id })
      .update({
        vaccancy: total_vacant,
      });


      if(isStudentidExists[0].parking_id){
   ...
   let deactivate_student_details = await knex("student_details")
    ...
    .update({
      ...
      room_id: 0,
      parking_id: 0,
      deactivate_date: moment()
        .utcOffset("+05:30")
        .format("YYYY-MM-DD HH:mm:ss"),
    });


    var studentDetail  = await knex("student_details")
    .where({
      id: req.body.id,
    });


    var subject ="Student "+studentDetail[0].SFname+" Deactivated";
    var message = "Dear RM Hostel Team, \n\n" +
      "This is to inform you that the student "+studentDetail[0].SFname+" has been deactivated from the portal successfully. Please visit the admin dashboard to get more details.\n\n" +
      "With Regards,\n\n" +
      "RM Hostel Team"
