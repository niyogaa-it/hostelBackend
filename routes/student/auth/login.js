...
//       });
//     }else if(selectedRows[0].is_approved !=1 || selectedRows[0].status !=1  ){
//       return res.json({
//         status: 401,
...
//         const message = "Dear Ms. "+selectedRows[0].SFname+", "+otp+" is your OTP for logging in. The OTP is valid for the next 5 minutes . Please do not share it with anyone. The hostel does not ask you to share OTP. - RMHostel";
//         const deliveredsms = await sentSms({ phone_number, msg: message });


       

//         const sendToRecipent = selectedRows[0].StudEmail;
    ...
    if (req.body.phone_number == undefined || req.body.phone_number == "") {
      ...
      if (selectedRows.length == 0) {
        ...
        return res.json({
          ...
          //Dont Delete it is is only for ios deployment
          if(req.body.phone_number =='9833600650'){
             ...
             var updateOtp = await knex("phone_auth").where( {
              ...
              const phone_number = req.body.phone_number;
              //const message = "Dear Ms. "+selectedRows[0].SFname+", "+otp+" is your OTP for logging in. The OTP is valid for the next 5 minutes . Please do not share it with anyone. The hostel does not ask you to share OTP. - RMHostel";
               const message = "Dear "+selectedRows[0].SFname+", Your login OTP is "+otp+". The OTP is valid for the next 5 minutes. Please do not share it with anyone. The hostel does not ask you to share OTP. -Chettinad";
              const deliveredsms = await sentSms({ phone_number, msg: message });
