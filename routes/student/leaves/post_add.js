...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (req.body.from === undefined || req.body.from === null) {
      ...
      const fromDate = moment(req.body.from, 'YYYY-MM-DD').format('YYYY-MM-DD HH:mm:ss');
      const toDate = moment(req.body.to, 'YYYY-MM-DD').format('YYYY-MM-DD HH:mm:ss');

      var user_dtl = await knex("student_details").where({
        id: req.user.id
      }).limit(1);
      ...
      var isSaveData = await knex("leaves_students").insert([
        {
          student_id: req.user.id,
          student_guardian_no: user_dtl[0].FatherConNo,
          student_name: req.user.SFname,
          from: fromDate,
          to: toDate,
