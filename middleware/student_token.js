        ...
        async function (err, verifiedUser) {
          if (err) {
            ...
            if (verifiedUser.user_id) {
              ...
              .select('student_details.id','student_details.SFname','student_details.SmobNo','student_details.StudEmail','student_details.food_preference','room.room_number as room_id')
              .leftJoin('room', 'student_details.room_id', 'room.id')
                .where("student_details.id", verifiedUser.user_id)
                .limit(1);

                if(user_details.length>0){

                  user_details[0].room_id = user_details[0].room_id;
                }
