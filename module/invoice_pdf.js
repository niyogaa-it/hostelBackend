...
function formatAcademicYear(dateInput) {
  if (!dateInput) return null;

  const date = moment(dateInput);
  const year = date.year();
  const month = date.month() + 1; // Jan = 1
...
async function invoiceGeneration(student_plan_id) {
    ...
    try {
          ...
          .where("student_plan.id", "=" ,student_plan_id).limit(1);
          ...
          if (gstIncluded == 'no') {
            tax_details[0].cgst = 0;
            tax_details[0].sgst = 0;
          }
          ...
          const createdAt = planDetail[0]?.paid_on
                ? moment(planDetail[0].paid_on).format("YYYY-MM-DD")
                : null;

          const fy = formatAcademicYear(createdAt);
          let get_last_plan_invoiceno = await knex("student_plan")
                                        .whereNotNull("invoice_no")
                                        .andWhere("invoice_no", "like", `RMH/${fy}/%`)
          ...
          let receipttotal_amount = 0;
          let receipttotal_amount_inwords = "";
          let refudable = 0;
          ...
          if (gstIncluded == 'yes') {
            ...
            roomrent_totalgst_in_percent = 0;
              ...
              if (gstIncluded == 'yes') {
                ...
                ori_baseprice_for_room = ori_baseprice_for_room;
          ...
          let total_taxable_amount = (parseFloat(taxable_amount_for_cgst) + parseFloat(taxable_amount_for_sgst)).toFixed(2);
          ...
          if((planDetail[0].plan_id == 1 || 
            ...
            if(planDetail[0].caution_deposit>0){
              total_amount = Math.round(parseInt(planDetail[0].total)-parseInt(planDetail[0].caution_deposit));
              total_amount_inwords = await inWords(total_amount);
              refudable = await inWords(planDetail[0].caution_deposit); 

              receipttotal_amount = Math.round(parseInt(planDetail[0]?.to_pay)-parseInt(planDetail[0]?.caution_deposit));
              receipttotal_amount_inwords = await inWords(receipttotal_amount);


            }else{
              total_amount = Math.round(parseInt(planDetail[0].total));
              total_amount_inwords = await inWords(total_amount);
            ...
            parkingrow =  planDetail[0].parking==0 ? '': `<tr align="center" valign="middle">
              <td colspan="2" style="text-align:center;font-family: Tahoma,Arial,sans-serif;padding: 5px 2px;">Parking ${planDetail[0]?.parking_type} Wheeler</td>
              <td style="text-align:center;font-family: Tahoma,Arial,sans-serif;padding: 5px 2px;">996322</td>
          ...
          if(planDetail[0].plan_type== 'prepaid'){
            ...
            total_amount = planDetail[0].total;
            total_amount_inwords = await inWords(total_amount);

            receipttotal_amount = planDetail[0]?.to_pay;
            receipttotal_amount_inwords = await inWords(planDetail[0]?.to_pay);
          ...
          if(planDetail[0].plan_id =='6'){
            ...
            let ori_baseprice_for_reg = (planDetail[0].total > 0) 
              ? planDetail[0].total / (1 + registrationfees_totalgst_in_percent) 
              : 0;

            let baseprice_for_reg = ori_baseprice_for_reg.toFixed(2);   
            let taxable_for_reg_cgst = (planDetail[0].total > 0) ? (ori_baseprice_for_reg * registrationfees_cgst_in_percent).toFixed(2) : 0;
            let taxable_for_reg_sgst = (planDetail[0].total > 0) ? (ori_baseprice_for_reg * registrationfees_sgst_in_percent).toFixed(2) : 0;
          ...
          if(planDetail[0].edit_plan_type == 'Meal change'){
           ...
           let file = {
            content: `
              ...
              }</p><p style="margin:0;font-size:10px;">ROOM NO : <span style="text-align:center;font-family: Tahoma,Arial,sans-serif;padding: 5px 2px;">${planDetail?.[0]?.room_number  || "NA"}</span></p><p style="margin:0;font-size:10px;">Address: ${
            ...
            <tr>
              ...
              <table width="100%" border="1" cellspacing="0" cellpadding="0" align="center" style="page-break-inside: avoid; break-inside: avoid;table-layout: fixed;border-collapse: collapse;border-spacing:0;font-family:Tahoma,Arial,sans-serif;color:#231f20;margin: auto;width: 100%;min-width:500px;max-width: 500px;border: 1px solid #000;background-color: #ffffff;padding: 10px 0;font-size: 6px;">
                <tbody><tr bgcolor="#45aadc">
                  <td colspan="2" align="center" style="text-align: center;font-family:Tahoma,Arial,sans-serif;padding: 10px 0;font-size: 14px;font-weight: bold;color: #000000;border-top: 1px solid #000;border-bottom: 1px solid #000;">RECEIPT</td>
                </tr><tr>
                  <td colspan="2" style="font-family: Tahoma,Arial,sans-serif;padding: 15px;">
                  <p  style="margin:0;font-size:9px;">Received with thanks from ${planDetail[0].SFname} a sum of Rs.${receipttotal_amount}/-</p><p  style="margin:0;font-size:9px;">(Rupees ${receipttotal_amount_inwords} only.)</p><p  style="margin:0;font-size:9px;">towards settlement of invoice no. ${invoiceNo} dated <span style="text-align:center;font-family: Tahoma,Arial,sans-serif;padding: 5px 2px;">${invoiceDate}</span></p></td>
                </tr>
          ...
          var updateInvoiceName = await knex("student_plan").where({id: student_plan_id}).update({
