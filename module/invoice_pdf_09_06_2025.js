...
async function invoiceGeneration(req) {
    ...
    try {
          ...
          let invoiceNo = 'RMH/24-25/'+ new_number;
          ...
          let roomrent_totalgst_in_percent = ((tax_details[0].cgst + tax_details[0].sgst) / 100);
          let admissionfees_totalgst_in_percent = ((tax_details[1].cgst + tax_details[1].sgst) / 100);
          let admissionkitfees_totalgst_in_percent = ((tax_details[2].cgst + tax_details[2].sgst) / 100);
          let culturalfees_totalgst_in_percent = ((tax_details[3].cgst + tax_details[3].sgst) / 100);
          let mealfees_totalgst_in_percent = ((tax_details[4].cgst + tax_details[4].sgst) / 100);
          let laundry_totalgst_in_percent = ((tax_details[5].cgst + tax_details[5].sgst) / 100);
          let parking_totalgst_in_percent = ((tax_details[6].cgst + tax_details[6].sgst) / 100);
          let transpotation_totalgst_in_percent = ((tax_details[7].cgst + tax_details[7].sgst) / 100)
          let waterfees_totalgst_in_percent = ((tax_details[8].cgst + tax_details[8].sgst) / 100);
          let electricityfees_totalgst_in_percent = ((tax_details[9].cgst + tax_details[9].sgst) / 100);
          let otherfees_totalgst_in_percent = ((tax_details[10].cgst + tax_details[10].sgst) / 100);
          let registrationfees_totalgst_in_percent = ((tax_details[11].cgst + tax_details[11].sgst) / 100);

          let roomrent_cgst_in_percent = (tax_details[0].cgst / 100);
          let admissionfees_cgst_in_percent = (tax_details[1].cgst / 100);
          let admissionkitfees_cgst_in_percent = (tax_details[2].cgst / 100);
          let culturalfees_cgst_in_percent = (tax_details[3].cgst / 100);
          let mealfees_cgst_in_percent = (tax_details[4].cgst / 100);
          let laundry_cgst_in_percent = (tax_details[5].cgst / 100);
          let parking_cgst_in_percent = (tax_details[6].cgst / 100);
          let transpotation_cgst_in_percent = (tax_details[7].cgst / 100);
          let waterfees_cgst_in_percent = (tax_details[8].cgst / 100);
          let electricityfees_cgst_in_percent = (tax_details[9].cgst / 100);
          let otherfees_cgst_in_percent = (tax_details[10].cgst / 100);
          let registrationfees_cgst_in_percent = (tax_details[11].cgst / 100);


          let roomrent_sgst_in_percent = (tax_details[0].sgst / 100);
          let admissionfees_sgst_in_percent = (tax_details[1].sgst / 100);
          let admissionkitfees_sgst_in_percent = (tax_details[2].sgst / 100);
          let culturalfees_sgst_in_percent = (tax_details[3].sgst / 100);
          let mealfees_sgst_in_percent = (tax_details[4].sgst / 100);
          let laundry_sgst_in_percent = (tax_details[5].sgst / 100);
          let parking_sgst_in_percent = (tax_details[6].sgst / 100);
          let transpotation_sgst_in_percent = (tax_details[7].sgst / 100);
          let waterfees_sgst_in_percent = (tax_details[8].sgst / 100);
          let electricityfees_sgst_in_percent = (tax_details[9].sgst / 100);
          let otherfees_sgst_in_percent = (tax_details[10].sgst / 100);
          let registrationfees_sgst_in_percent = (tax_details[11].sgst / 100);


          let ori_baseprice_for_room =  (planDetail[0].room_rent !== null) ?  (planDetail[0].room_rent/(1+roomrent_totalgst_in_percent) ) : 0 ; 
          let ori_baseprice_for_admission_fee =  (planDetail[0].addmission_fee !== null) ? (planDetail[0].addmission_fee /(1 + admissionfees_totalgst_in_percent)) : 0 ;  
          let ori_baseprice_for_admisson_kit =  (planDetail[0].admisson_kit!== null) ?  (planDetail[0].admisson_kit /(1 + admissionkitfees_totalgst_in_percent) ) : 0 ;  
          let ori_baseprice_for_cultural_fees = (planDetail[0].cultural_fees!== null) ? (planDetail[0].cultural_fees /(1+ culturalfees_totalgst_in_percent)) : 0 ;  
          let ori_baseprice_for_meal =  (meal!== null) ?  (meal /(1+ mealfees_totalgst_in_percent) ) : 0 ; 
          let ori_baseprice_for_laundry =    (laundry!== null) ? (laundry/(1+ laundry_totalgst_in_percent) ) : 0 ; 
          let ori_baseprice_for_parking = (planDetail[0].parking!== null) ? (planDetail[0].parking /(1+ parking_totalgst_in_percent)) : 0 ; 
          let ori_baseprice_for_transportation =  (planDetail[0].transportation!== null) ? (planDetail[0].transportation /(1+ transpotation_totalgst_in_percent )) : 0 ; 
          let ori_baseprice_for_wc =    (planDetail[0].monthly_water_bill>0) ? (planDetail[0].monthly_water_bill/(1+ waterfees_totalgst_in_percent) ) : 0 ; 
          let ori_baseprice_for_eb =    (planDetail[0].monthly_electricity_bill>0) ? (planDetail[0].monthly_electricity_bill/(1+ electricityfees_totalgst_in_percent) ) : 0 ; 
          let ori_baseprice_for_other =    (planDetail[0].monthly_other_fees>0) ? (planDetail[0].monthly_other_fees/(1+ otherfees_totalgst_in_percent) ) : 0;
          

          let baseprice_for_room =  (planDetail[0].room_rent !== null) ?  ori_baseprice_for_room.toFixed(2) : 0 ; 
          let baseprice_for_admission_fee =  (planDetail[0].addmission_fee !== null) ? ori_baseprice_for_admission_fee.toFixed(2) : 0 ;  
          let baseprice_for_admisson_kit =  (planDetail[0].admisson_kit!== null) ?  ori_baseprice_for_admisson_kit.toFixed(2) : 0 ;  
          let baseprice_for_cultural_fees = (planDetail[0].cultural_fees!== null) ? ori_baseprice_for_cultural_fees.toFixed(2) : 0 ;  
          let baseprice_for_meal =  (meal!== null) ?  ori_baseprice_for_meal.toFixed(2) : 0 ; 
          let baseprice_for_laundry =    (laundry!== null) ? ori_baseprice_for_laundry.toFixed(2) : 0 ; 
          let baseprice_for_parking = (planDetail[0].parking!== null) ? ori_baseprice_for_parking.toFixed(2) : 0 ; 
          let baseprice_for_transportation =  (planDetail[0].transportation!== null) ? ori_baseprice_for_transportation.toFixed(2) : 0 ; 
          let baseprice_for_wc =    (planDetail[0].monthly_water_bill>0) ? ori_baseprice_for_wc.toFixed(2) : 0 ; 
          let baseprice_for_eb =    (planDetail[0].monthly_electricity_bill>0) ? ori_baseprice_for_eb.toFixed(2) : 0 ; 
          let baseprice_for_other =    (planDetail[0].monthly_other_fees>0) ? ori_baseprice_for_other.toFixed(2) : 0 ;
          

          //calculate cgst
          let taxable_for_room_cgst = (planDetail[0].room_rent !== null) ? (ori_baseprice_for_room * roomrent_cgst_in_percent).toFixed(2) : 0; 
          let taxable_for_admission_fee_cgst = (planDetail[0].addmission_fee !== null) ? (ori_baseprice_for_admission_fee * admissionfees_cgst_in_percent).toFixed(2) : 0; 
          let taxable_for_admisson_kit_cgst = (planDetail[0].admisson_kit !== null) ? (ori_baseprice_for_admisson_kit * admissionkitfees_cgst_in_percent).toFixed(2) : 0; 
          let taxable_for_cultural_fees_cgst = (planDetail[0].cultural_fees !== null) ? (ori_baseprice_for_cultural_fees * culturalfees_cgst_in_percent).toFixed(2) : 0; 
          let taxable_for_meal_cgst = (meal !== null) ? (ori_baseprice_for_meal * mealfees_cgst_in_percent).toFixed(2) : 0; 
          let taxable_for_laundry_cgst = (laundry !== null) ? (ori_baseprice_for_laundry * laundry_cgst_in_percent).toFixed(2) : 0; 
          let taxable_for_parking_cgst = (planDetail[0].parking !== null) ? (ori_baseprice_for_parking * parking_cgst_in_percent).toFixed(2) : 0; 
          let taxable_for_transportation_cgst = (planDetail[0].transportation !== null) ? (ori_baseprice_for_transportation * transpotation_cgst_in_percent).toFixed(2) : 0;
          let taxable_for_wc_cgst = (planDetail[0].monthly_water_bill > 0) ? (ori_baseprice_for_wc * waterfees_cgst_in_percent).toFixed(2) : 0;
          let taxable_for_eb_cgst = (planDetail[0].monthly_electricity_bill > 0) ? (ori_baseprice_for_eb * electricityfees_cgst_in_percent).toFixed(2) : 0;
          let taxable_for_other_cgst = (planDetail[0].monthly_other_fees > 0) ? (ori_baseprice_for_other * otherfees_cgst_in_percent).toFixed(2) : 0;


          //calculate sgst
          let taxable_for_room_sgst = (planDetail[0].room_rent !== null) ? (ori_baseprice_for_room * roomrent_sgst_in_percent).toFixed(2) : 0; 
          let taxable_for_admission_fee_sgst = (planDetail[0].addmission_fee !== null) ? (ori_baseprice_for_admission_fee * admissionfees_sgst_in_percent).toFixed(2) : 0; 
          let taxable_for_admisson_kit_sgst = (planDetail[0].admisson_kit !== null) ? (ori_baseprice_for_admisson_kit * admissionkitfees_sgst_in_percent).toFixed(2) : 0; 
          let taxable_for_cultural_fees_sgst = (planDetail[0].cultural_fees !== null) ? (ori_baseprice_for_cultural_fees * culturalfees_sgst_in_percent).toFixed(2) : 0; 
          let taxable_for_meal_sgst = (meal !== null) ? (ori_baseprice_for_meal * mealfees_sgst_in_percent).toFixed(2) : 0; 
          let taxable_for_laundry_sgst = (laundry !== null) ? (ori_baseprice_for_laundry * laundry_sgst_in_percent).toFixed(2) : 0; 
          let taxable_for_parking_sgst = (planDetail[0].parking !== null) ? (ori_baseprice_for_parking * parking_sgst_in_percent).toFixed(2) : 0; 
          let taxable_for_transportation_sgst = (planDetail[0].transportation !== null) ? (ori_baseprice_for_transportation * transpotation_sgst_in_percent).toFixed(2) : 0;
          let taxable_for_wc_sgst = (planDetail[0].monthly_water_bill > 0) ? (ori_baseprice_for_wc * waterfees_sgst_in_percent).toFixed(2) : 0;
          let taxable_for_eb_sgst = (planDetail[0].monthly_electricity_bill > 0) ? (ori_baseprice_for_eb * electricityfees_sgst_in_percent).toFixed(2) : 0;
          let taxable_for_other_sgst = (planDetail[0].monthly_other_fees > 0) ? (ori_baseprice_for_other * otherfees_sgst_in_percent).toFixed(2) : 0;

          let total_base_price = (ori_baseprice_for_room + ori_baseprice_for_admission_fee + ori_baseprice_for_admisson_kit + ori_baseprice_for_cultural_fees + ori_baseprice_for_meal + ori_baseprice_for_laundry + ori_baseprice_for_parking + ori_baseprice_for_transportation + ori_baseprice_for_wc + ori_baseprice_for_eb+ ori_baseprice_for_other).toFixed(2);
