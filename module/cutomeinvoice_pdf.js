...
async function customInvoiceGeneration(planId,room_number) {
    ...
    try {
          ...
          .where("student_plan.id", "=" ,planId).limit(1);
          ...
          if (planDetail && planDetail.length > 0) {
            ...
            <td colspan="2" style="text-align:center;font-family: Tahoma,Arial,sans-serif;padding: 5px 2px;">${planDetail[0]?.monthly_other_fees_remark ?? 'Other Changes'}</td>
            <td style="text-align:center;font-family: Tahoma,Arial,sans-serif;padding: 5px 2px;">996322</td>
          ...
          if(planDetail[0]?.edit_plan_type == 'Meal change'){
            ...
            total_amount_inwords = await inWords(total_amount);
              ...
              var updateInvoiceName = await knex("student_plan").where({id: planId}).update({
...
module.exports = {customInvoiceGeneration};
